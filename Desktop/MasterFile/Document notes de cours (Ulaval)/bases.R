...
## construisent les tables de vérité du «et» logique et du
## «ou» logique.]
...
## Les fonctions 'as.numeric', 'as.logical' et 'as.character'
## sont utiles pour forcer la conversion d'un mode vers un
## autre.
...
c(0, 3, -1, 4) > 0         # inégalité

## Il n'est pas possible d'écrire des inégalités doubles du
## type 0 < x < 10 en R (comme, par ailleurs, dans la plupart
## des langages de programmation). L'inégalité précédente se
## lit: «x > 0 ET x < 10». Pour calculer l'inégalité double,
## il s'agit donc de calculer les deux inégalités et de les
## relier par un «et» logique qui fera le calcul élément par
## élément.
x <- c(-2, 3, 0, -1, 12, 8) # vecteur
0 < x                       # première inégalité
x < 10                      # seconde inégalité
0 < x & x < 10              # inégalité double
x <= 0 | x >= 10            # négation de l'inégalité
...
c(-2, 3, -1, 4) <= 1:4     # quatre inégalités différentes
c(TRUE, FALSE) | FALSE     # deux «ou» logiques
...
x[0 < x & x < 10]          # autre extraction par critère
...
## Supprimons la fonction 'abs' pour éviter qu'elle entre
## en conflit avec la fonction interne de R du même nom.
rm("abs")

## L'exemple suivant vise à vous faire réaliser que les
## valeurs booléennes constituent implicitement des résultats
## de tests logiques.
##
## Nous voulons écrire une fonction 'is.odd' qui détermine si
## les éléments d'un vecteur sont impairs ou non. Un nombre
## est impair si le reste de sa division par 2 (le modulo) est
## égal à 1.
##
## Voici une première mise en oeuvre naïve.
is.odd <- function(x)
{
    if (x %% 2 == 1)
        TRUE
...
## Le résultat de 'x %% 2' est nécessairement 0 ou 1. Ces
## valeurs étant automatiquement converties en FALSE et TRUE
## dans une opération logique, nous pouvons les utiliser
## directement à l'intérieur de la condition d'une clause 'if'
## sans tester leur égalité avec 0 ou 1.
##
## De plus, comme le résultat d'une condition est
## nécessairement TRUE ou FALSE, la construction ci-dessus
## revient à dire: «si la condition est vraie, retourner VRAI,
## sinon retourner FAUX». N'est-ce pas redondant?
##
## Enfin, la mise en oeuvre ci-dessus n'est pas vectorielle
## puisqu'il ne peut y avoir qu'un seul test à l'intérieur de
## la clause 'if'.
is.odd(c(0, 4, 3, 6, 8, 9, 1))

## Avant d'étudier une meilleure mise en oeuvre, voyons
## comment ça aurait pu être pire en testant explicitement
## 'TRUE == TRUE'.
is.odd <- function(x)
{
    if ((x %% 2 == 1) == TRUE)
...
## Ou pire encore en utilisant une logique tordue ou l'on
## retourne FAUX lorsque la condition est vraie et VRAI
## lorsqu'elle est fausse! (Je n'invente rien, c'est un cas
## rencontré.)
is.odd <- function(x)
{
    if ((x %% 2 == 0) == TRUE)
...
## En fait, la solution simple, élégante et vectorielle
## retourne simplement les résultats de l'opération modulo
## convertis en valeurs booléennes, chacune constituant
## implicitement le résultat d'un test logique.
...
## Une chaine de caractères est un regroupement entre
## guillemets " " de 0, 1 ou plusieurs caractères. Puisque
## tout objet dans R est un vecteur, une chaine de caractères
## seule est un vecteur contenant une seule chaine de
## caractères. Un vecteur peut également contenir plusieurs
## chaines de caractères de longueurs différentes.
"foo"                      # vecteur d'une seule chaine
c("", "foo", "foobar")     # vecteur de 3 chaines

## Les fonctions ci-dessous permettent de manipuler une chaine
## de caractères. Elles sont vectorielles, c'est-à-dire
## qu'elles peuvent effectuer leur traitement sur chaque
## chaine d'un vecteur.
s <- c("Programmation", "analyse", "données")
paste(1:3, "-", s)         # création de 3 chaines
paste0(1:3, "-", s)        # idem, sans espace
paste0(s, collapse = " ")  # concaténation des 3 chaines
nchar(s)                   # nombres de caractères
substr(s, 2, 5)            # sous-chaines
strsplit("2001-05", "-")   # séparation en sous-chaines  
